...
called a polling station), possibly wait in line before authenticating  
...
interest here is the second of the steps noted above: waiting to check in at a polling station.

We draw attention to two important features of voting lines, and by
voting lines here we mean lines that form in front of polling stations as
opposed to lines that affect voters already in the act of casting
ballots.  First, waiting in line to vote constitutes a "time tax" \cite{mukherjee:timetax}. This tax can be negligible (for
...
may opt for an alternative eatery, one without
...
the likelihood of voting in future elections.
...
and election administration: voter check-in times. Specifically, we draw on data collected by Florida
...
his or her race/ethnicity, party registration, and electoral participation
history.  However, in the interests of being open about limitations in
our data, a voter's check-in time is not connected directly to the 
...
Before turning to our results, we situate our study in the literature on
...
and the second on the effects of the tax.  We then provide some concluding thoughts for future research on wait times.

\section*{Voting lines in American elections}

The literature on voting lines is growing, and most of its dates to
recent elections.  With few exceptions, what we know about
...
\cite{stewart:waitingtovote2012} finds that a voter's race/ethnicity is an
...
processing completed ballots. Third, given the 
variability across states and local jurisdictions in election administration practices,
national surveys, including the SPAE, may not be not particularly well-suited for assessing
...
the study of election administration. However, with the
...
understand why lines form inside Florida polling stations 
...
largely controlled at the county level within a statutory framework established
by the Florida legislature and administrative rules promulgated by the state Division of Elections. 
The primary data source in our analysis is thus gleaned from Florida counties, and it consists of early voting
...
This is no question that across the American states early voting has become increasingly popular; for
...
EViD machines allow county pollworkers to instantaneously check-in and verify the
...
county's early voting centers and interfaces with the Florida statewide
voter database, thwarting potential double-voting.  More
...
hence are presumably more accurate than surveys asking voters about their time waiting in line or even
within-polling place observations of voting queues.
...
voters who checked-in at these two polling stations.  The counts begin
...
self-reported race/ethnicity, and these details are depicted via bar colors.
...
early voting locations---polling stations that voters registered in a
given county are permitted to cast early ballots---that serviced a
...
check-in.  For most of the day, whites were the majority racial/ethnic group,
...
For three key race/ethinic groups in Florida, black, Hispanic, and white, the
